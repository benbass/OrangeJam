  ...
  Widget build(BuildContext context) {
    ...
    void reorderItem(int oldIndex, int newIndex, PlaylistsState state) {
      ...
      if (state.playlistId > -1) {
        state.playlists[state.playlistId][1].remove(track.filePath);
        state.playlists[state.playlistId][1].insert(newIndex, track.filePath);

        PlaylistHandler().reorderLinesInFile(
            state.playlists[state.playlistId][0], oldIndex, newIndex);
      }
    }
                        ...
                        builder: (context, orientation) {
                          if (orientation == Orientation.portrait) {
                            return AnimatedContainer(
                              ...
                              height:
                                  playerControlsState.height == 200 ? 200 : 0,
                            );
                          } else {
